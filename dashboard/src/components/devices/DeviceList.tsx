				...
				return (
					<div
						...
						<div className='flex-1 min-w-0'>
							<p className={`text-[11px] ${text.primary} truncate`}>{device.label}</p>
							<p className={`text-[10px] ${text.dim} ${t.mono} truncate`}>{device.device_id}</p>
						</div>
						<span className={`text-[10px] shrink-0 ${online ? 'text-green-700' : text.dim}`}>{online ? 'Online' : `Last seen ${formatTimestamp(device.last_seen_at, 'session')}`}</span>
					</div>
				);
			})}
		</div>
